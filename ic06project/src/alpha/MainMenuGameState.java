...
public class MainMenuGameState extends BasicGameState implements MouseListener {
	...
	public void init(GameContainer gc, StateBasedGame sbg)
		...
		mapImage = new Image(Global.PATH_IMAGES_RESSOURCES+"menu_map.jpg");
	...
	public void mouseClicked(int button, int x, int y, int clickCount){
		...
		else if((x >= LOADGAME_X && x <= (LOADGAME_X + newGameImage.getWidth())) 
				&&	(y >= LOADGAME_Y && y <= (LOADGAME_Y + newGameImage.getHeight()))){
			selection = Game.LOADGAME_STATE;
		}
		else if(optionsActivated&& (x >= OPTIONS_X && x <= (OPTIONS_X + newGameImage.getWidth())) 
				&&	(y >= OPTIONS_Y && y <= (OPTIONS_Y + newGameImage.getHeight()))){
